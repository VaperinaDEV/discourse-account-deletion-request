export default function migrate(settings) {
    const stringSettingNames = [
      "dropdown_button_icon",
      "request_dropdown_item_icon",
      "info_dropdown_item_icon",
    ];
  
    const listSettingName = "svg_icons";
  
    // Handle string settings
    stringSettingNames.forEach((settingName) => {
    ...
    // Handle list setting
    const oldListSetting = settings.get(listSettingName);
    if (oldListSetting) {
      const newListSetting = oldListSetting
        .split("|")
        .map((icon) => convertIconName(icon))
        .join("|");
  
      if (oldListSetting !== newListSetting) {
        settings.set(listSettingName, newListSetting);
      }
    }
  ...
  function convertIconName(iconName) {
    const fa5_remaps = {
      ...
      if (fa5_remaps.hasOwnProperty(lookupName)) {
        const remappedName = fa5_remaps[lookupName];
